<x-admin-layout title="Chỉnh sửa Mã khuyến mãi">
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-black text-slate-900">Cập nhật chương trình</h2>
    </div>

    <form action="{{ route('admin.vouchers.update', $voucher) }}" method="POST" class="max-w-3xl">
        @csrf
        @method('PUT')
        <div class="bg-white p-10 rounded-[3rem] shadow-xl shadow-slate-200/50 border border-slate-100 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="text" name="code" value="{{ $voucher->code }}" required placeholder="Ví dụ: SUMMER2026" 
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="discount_percent" value="{{ $voucher->discount_percent }}" min="1" max="100" required 
            ...
            <div class="space-y-2">
                ...
                <input type="number" name="min_amount" value="{{ $voucher->min_amount }}" required 
                       class="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-6 py-4 text-lg font-black focus:border-blue-600 focus:outline-none transition">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="datetime-local" name="start_date" value="{{ $voucher->start_date->format('Y-m-d\TH:i') }}" required 
                ...
                <div class="space-y-2">
                    ...
                    <input type="datetime-local" name="end_date" value="{{ $voucher->end_date->format('Y-m-d\TH:i') }}" required 
                           class="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-6 py-4 font-bold focus:border-blue-600 focus:outline-none transition">
                </div>
            </div>
            ...
            <div class="flex items-center gap-4 p-6 bg-slate-50 rounded-2xl">
                ...
                <input type="checkbox" name="is_active" value="1" {{ $voucher->is_active ? 'checked' : '' }} id="is_active" class="w-6 h-6 rounded-lg border-2 border-slate-200 text-blue-600 focus:ring-blue-600">
                <label for="is_active" class="text-sm font-black text-slate-700">Kích hoạt mã khuyến mãi</label>
            </div>
