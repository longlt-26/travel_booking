<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="font-black text-2xl text-slate-900 leading-tight">Quản lý Tour Du Lịch</h2>
                <p class="text-slate-500 text-sm mt-1">Danh sách tất cả các tour đang kinh doanh trên hệ thống.</p>
            </div>
            <a href="{{ route('admin.tours.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-blue-200 transition-all active:scale-95 flex items-center gap-2 w-fit">
    ...
    <script>
        document.querySelectorAll('.delete-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                Swal.fire({
                    title: 'Xóa tour này?',
                    text: "Dữ liệu đã xóa sẽ không thể khôi phục lại!",
                    icon: 'warning',
