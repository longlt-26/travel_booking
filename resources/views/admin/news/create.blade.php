<x-admin-layout title="Thêm Tin tức mới">
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-black text-slate-900">Tạo bài viết mới</h2>
    </div>

    <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        @csrf
            ...
            <div class="bg-white p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="summary" rows="3" placeholder="Mô tả ngắn gọn nội dung bài viết..."
                              class="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-6 py-4 focus:border-blue-600 focus:outline-none transition font-medium"></textarea>
                </div>
